...
- (NSInteger)pickerView:(UIPickerView *)pickerView numberOfRowsInComponent:(NSInteger)component {
    ...
    switch (component) {
        case 0:
            //省
            return manager.areaTree.count;
            break;
        case 1:
            //市
        {
            NSArray *shiArr = [manager.areaTree[self.selectShengInt] objectForKey:@"item"];
            return shiArr.count;
...
- (UIView *)pickerView:(UIPickerView *)pickerView viewForRow:(NSInteger)row forComponent:(NSInteger)component reusingView:(UIView *)view {
    ...
    switch (component) {
        case 0:
            //省
            titleStr = [manager.areaTree[row] objectForKey:@"a_name"];
            break;
        case 1:
            //市
        {
...
- (IBAction)enterSelectAreaButtonAction:(UIButton *)sender {
    ...
    //地区编码记录一下
    //得到省名称
    NSString *shengStr = [manager.areaTree[self.selectShengInt] objectForKey:@"a_name"];
    //得到市数组
    NSArray *shiArr = [manager.areaTree[self.selectShengInt] objectForKey:@"item"];
    //得到市名称
